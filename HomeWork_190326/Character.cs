...
namespace HomeWork_190326
{
    public class Character
    {
        ...
        //装备一件装备,grabByForce-强行抢夺已经被其他角色装备的装备
        public void Equip(Equipment equipment, bool grabByForce)
        {
            //装备没有被其他角色拥有的时候,直接装备
            if(equipment.owmer == null)
            {
                //如果当前有装备其他装备,要先把当前装备的装备所有者设为空,再更换装备
                if(HasEquipment())
                {
                    this.equipment.owmer = null;
                }
              
                this.equipment = equipment;
                equipment.owmer = this;
            ...
            else  //装备已经被其他角色拥有的时候,如果“强行抢夺”为false,就无法装备,否则让其他角色卸下装备,装备到自己身上
            {
                if(grabByForce == false)
                {
                    Console.WriteLine("{0}已经被{1}拥有,不能被{2}装备", equipment.name, equipment.owmer.name, this.name);
                }
                else
                {
                    equipment.owmer.UnEquip();
                    this.equipment = equipment;
                    equipment.owmer = this;
